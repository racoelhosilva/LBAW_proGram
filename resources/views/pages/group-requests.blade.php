...
@section('content')
    <main id="group-requests-page" class="px-8 py-4 flex flex-col gap-6" data-group-id={{$group->id}}>
        <section id="requests" class="flex flex-col w-full gap-4">
            <h1 class="text-2xl text-center font-medium m-4">Join Requests</h1>
            @foreach ($usersWhoWantToJoin as $user)
            ...
            @endforeach
            <p id="no-requests-indicator" class="{{ $usersWhoWantToJoin->isEmpty() ? '' : 'hidden' }}">No requests at the moment</p>
        </section>
      </main>
@endsection
