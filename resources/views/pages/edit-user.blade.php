    ...
    <form method="post" action="{{ route('user.update', $user->id) }}" class="grid gap-4 justify-self-stretch" id="profile-form">
        {{ csrf_field() }}
            ...
            <section id="visibility" class="flex items-center mt-4">
                <input type="checkbox" id="is-public" name="is_public" value="1" class="w-5 h-5 mr-2 text-blue-600 border-gray-300 rounded focus:ring-blue-500 focus:ring-2" {{ $user->is_public ? 'checked' : '' }}>
